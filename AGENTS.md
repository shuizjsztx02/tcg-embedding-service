...
- 远程仓库：origin → https://github.com/shuizjsztx02/tcg-embedding-service.git
...
6. **每次 Task 完成后提交并推送 GitHub**：完成一个可独立验收的 Task 后，仅暂存本任务的源码、配置和文档，检查暂存区后 git commit 并 git push；用户明确要求停止或不推送时，不得继续推送。不要用未经检查的 git add -A 混入已有改动或数据文件。commit message 需简明描述做了什么。
7. 不要尝试把images的所有图片全部读入，占用上下文资源，你只需要知道这里是训练使用的图片即可
8. **数据文件禁止纳入 Git**：测试图片、测试数据、采集数据、自动生成结果（含索引、下载/生成的模型文件和缓存）均保留在本地并加入 .gitignore，不得跟踪或提交。已经跟踪的数据应仅从 Git 索引移除，不能删除本地文件。提交前检查 git ls-files 和暂存区；对于未推送提交中已加入的数据，须一并清理未推送历史，避免下次 push 仍然上传。若历史清理受安全审查阻止，保持停止推送并明确说明阻塞，不得绕过限制。不得擅自改写远端历史或强推。
---
...
- 方向：8888 张竖版、546 张横版（疑似旋转 90 度的竖卡）
...
### 索引
- index/embeddings.npy：9434 x 768 float32，L2 归一化
...
5. **拒绝策略**：top-1 分数 >= tau 且 top-1 与 top-2 分差 >= m，tau 和 m 通过校准确定。
6. **双方向推理**：横版图同时尝试 90 度和 270 度旋转，取最高分方向。
...
1. **51 组重复 SHA1**：不同卡号共用同一张图，模型无法区分，需要标记。
2. **200px 分辨率上限**：不支持读取卡号/系列标志等细粒度信息，可能成为准确率天花板。
3. **无 GPU**：当前环境 CPU-only，微调需要阿里云 GPU。
4. **无真实照片数据**：阈值校准只能用模拟数据，上线后可能需要调整。
