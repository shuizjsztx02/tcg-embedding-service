 ...
 |------|------|--------|
 | M0：数据治理 | **已完成** | 100% |
 | M1：裸 DINOv2 基线 | 已完成 | 100% |
 | M2：API 服务 | **已完成** | 100% |
 | M3：微调（需 GPU） | 未开始 | 0% |
 | M4：真实照片验收 | 未开始 | 0% |
 
 **当前焦点**：Task 7 API 服务已完成 → 下一步微调或真实照片验收
 ...
 **验收标准**：
 - [x] 修复后的 persp_coeffs 在非恒等透视变换下能正确 warp 图片
 - [x] 基线评估脚本可完整运行（无报错）
 - [x] 产出 top-1 准确率、genuine/impostor 分数分布
 - [x] 产出 baseline_eval.csv
 ...
 **验收标准**：
 ...
 - [x] 重复图片清单已产出
 - [x] 横版图方向确认结果已记录
 - [x] .gitignore 已创建（忽略 images/、index/、csv/*.csv、__pycache__、*.pth）
 - [x] 产出 csv/corrupt_ids.txt
 - [x] 产出 csv/duplicate_groups.csv
 - [x] 抽样验证横版图方向（546 张 landscape 均为旋转 90° 的竖版卡）
 ...
 **验收标准**：
 - [x] index/ 目录下产出 embeddings.npy（9434×768 float32）和 ids.json
 - [x] 自检 100 张图 top-1 命中率 100%
 - [x] 索引构建时间已记录
 ...
 **验收标准**：
 - [x] 基线数字已产出（top-1 准确率、genuine/impostor 分数分布）
 - [x] 混淆分析报告已产出
 - [x] 已知卡准确率 + 未知卡拒绝率已记录
 ...
 **建议**：从现在开始收集，不阻塞任何技术工作
 ...
 | 2026-08-26 | GitHub 仓库建立（origin → shuizjsztx02/tcg-embedding-service） | 版本管理，支持回滚 | 用户 |
 | 2026-08-26 | 每次 Task 完成后提交并推送 GitHub | 方便出问题时回滚到稳定版本 | 用户 |
 ...
 | 2026-08-26 | v1.1 | 添加版本管理规范（GitHub 仓库 + 每次 Task 后提交推送） | Codex |
 | 2026-08-26 | v1.2 | Task 2 数据治理完成：产出 corrupt_ids.txt、duplicate_groups.csv，确认所有 landscape 图为旋转竖版卡 | Codex |
 | 2026-08-26 | v1.3 | Task 3 索引构建完成：embeddings.npy（9434×768 float32），ids.json，自检 100/100 命中，构建耗时 943.7s | Codex |
 | 2026-08-26 | v1.4 | Task 4 正式基线评估完成：top-1 准确率 81.0%（405/500），真实照片分数分布显著偏低，继续推进 API 服务 | Codex |
